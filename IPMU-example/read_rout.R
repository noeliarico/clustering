...
iter <- 0
check_centers <- TRUE
distance_names <- c(distances[selected_distances], "bordaCount")
for(line in lines) {
  current_distance <- distance_names[distance]
  ...
  if(str_detect(line, "\\[p")) {
    ...
    if(check_centers) {
      ...
      cx <- c[1]
      cy <- c[2]
      iters_info <- iters_info %>% add_row(distance = current_distance, iter, point = center_name, cluster = str_remove(center_name, "c"), x = cx, y = cy)
    }
  ...
  if(str_detect(line, "assigned to cluster")) {
    ...
    iters_info <- iters_info %>% add_row(distance = current_distance, iter, point, cluster, x = 0, y = 0)
  }
}
